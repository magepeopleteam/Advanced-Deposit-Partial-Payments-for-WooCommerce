...
class MEP_PP_Cart
{
    ...
    public function add_cart_item_pp_deposit_data($cart_item_data, $product_id)
    {
        ...
        // From Shop Page
        if(!isset($deposit_mode)) {
            $is_partial_enable = get_post_meta($product_id, '_mep_enable_pp_deposit', true);
            if(!$is_partial_enable) {
                $is_partial_enable = get_option('mepp_enable_partial_by_default') ? get_option('mepp_enable_partial_by_default') : 'no';
            }

            if($is_partial_enable !== 'yes') {
                return $cart_item_data;
            }
        ...
        if (meppp_is_product_type_pp_deposit($product_id) == false || $deposit_mode == 'check_full') {
            return $cart_item_data;
        }
